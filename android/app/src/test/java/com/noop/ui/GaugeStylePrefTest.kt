...
/**
 * The app-wide gauge-style preference: rings by default, vessels when turned off.
 *
 ...
 * Today screens there; this chooses a gauge style app-wide on Android, and binding the two together
 * would make a later divergence on either platform quietly wrong.
 */
@RunWith(RobolectricTestRunner::class)
class GaugeStylePrefTest {
    ...
    fun `an install that never opened settings gets the rings`() {
        val c = ctx()
        NoopPrefs.of(c).edit().remove(NoopPrefs.KEY_RING_GAUGES).apply()
        assertTrue("rings are what #2311 shipped, so the absent key must read as true",
                   NoopPrefs.ringGauges(c))
    }

    @Test
    fun `turning it off selects the vessels and survives a read`() {
        val c = ctx()
        NoopPrefs.setRingGauges(c, false)
        assertFalse(NoopPrefs.ringGauges(c))
        NoopPrefs.setRingGauges(c, true)
        assertTrue(NoopPrefs.ringGauges(c))
    }
    ...
    fun `the key is its own, not Apple's whole-screen toggle`() {
        ...
        assertTrue(NoopPrefs.KEY_RING_GAUGES == "noop.ringGauges")
        assertFalse(NoopPrefs.KEY_RING_GAUGES == "noop.liquidTodayEnabled")
    }
}
